...
from member.schemas.users_schema import (  # Pydantic 모델을 임포트
    PasswordUpdateRequest, UserCreateRequest, UserDeleteRequest,
    UserImgUpdateRequest, UserInfoResponse, UserLoginRequest)
...
def login_view(request, login_user: UserLoginRequest):
    ...
    user = authenticate(
        username=l_user["user_id"], password=l_user["password"]
    )  # 사용자를 인증합니다.
    ...
    if user is not None:
        ...
        return {"sessionid": sessionid}, 200  # 세션 ID를 클라이언트에게 반환합니다.

    return {"detail": "Invalid credentials"}, 200
